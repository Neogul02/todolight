...
- **모바일에서는 순환한다** (`hooks/useLoopCarousel.ts`). 내 컬럼에서 왼쪽으로 밀면 맨 끝
  ...
  - 데스크톱(`sm:` 이상)은 순환하지 않는다 — 컬럼이 여러 개 동시에 보여서 되감기면 화면이
    뚝 끊긴 것처럼 보인다.
  - 첫 슬라이드로 맞추는 건 **최초 1회와 `resetKey`(조직·뷰)가 바뀔 때뿐이다.** 멤버 수가
    바뀔 때마다 되돌리면, 팀원 한 명 들어왔다고 남의 컬럼 보던 사람이 자기 컬럼으로 끌려간다.
  - **위치 잡기는 한 번에 성공한다고 가정하면 안 된다.** 뷰 전환이 `AnimatePresence mode="wait"`라
    이전 뷰의 exit(0.2초)이 끝나야 컬럼이 DOM에 붙는다. 그전에 시도하면 조용히 실패하는데,
    그걸 성공으로 치고 `positioned`를 세우면 스크롤이 0에 남고 — loop에서 0번 슬라이드는
    **마지막 팀원의 복제본**이라 보드를 열 때마다 엉뚱한 사람이 보인다.
    `jumpTo`는 성공 여부를 반환하고, 붙을 때까지 프레임 단위로 다시 시도한다.
