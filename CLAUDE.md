...
- **429는 계정 단위 분당 120회**(`20260908130018_api_rate_limit.sql`). 서버리스는 요청마다
  다른 인스턴스에서 돌 수 있어 프로세스 메모리로는 아무것도 셀 수 없다 — Redis를 새로 붙이는
  대신 이미 있는 Postgres에 고정 창을 둔다. **제한 조회가 실패하면 통과시킨다**(fail-open):
  실패하는 건 DB가 아플 때인데 그때 API를 통째로 닫으면 장애가 몇 배가 된다.
  인증은 그 위에서 이미 끝났으므로 열어 줘도 남의 데이터로는 못 간다.
- 미인증 경로는 **DB를 건드리지 않는다** — 토큰 검증이 JWKS 로컬이라, 토큰 없이 두들기는
  요청이 DB까지 내려가지 않는다. rate limit을 인증 뒤에 둬도 구멍이 되지 않는 이유다.

**엔드포인트 표와 계약은 `docs/api-v1.md`에 있다 — Swift 모델의 근거는 그 문서 하나여야 한다.**
엔드포인트를 더하면 그 표에도 줄을 더할 것(라우트와 문서가 1:1인지 대조할 수 있어야 한다).
