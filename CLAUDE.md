...
- **이름은 시작일과 주의 첫날에만** 얹는다. 칸 하나에 가두면 두 글자도 못 넣으므로,
  그 주에 이어지는 칸 수만큼 폭을 늘려(`calc(100% * n + 4px * (n-1))`) 옆 칸 위로 흘려보낸다.
  이름을 얹은 띠는 `z-10` — 안 그러면 나중에 그려지는 옆 칸 띠가 덮는다.
...
펼침 상태는 `BoardClient`의 `openTodoId` 하나뿐이다. 여러 개가 동시에 열려 있으면 컬럼이
