...
펼침 상태는 `BoardClient`의 `openTodo` 하나뿐이고 **어느 뷰에서 열었는지도 같이 담는다**.
뷰 전환이 헤더에서 일어나서 보드가 그 변화를 감지할 수 없기 때문에, 렌더할 때 뷰를 비교해
...
테마는 6개다 — **시스템(기본)** · 화이트 · 샌드 · 블랙 · 미드나잇 · 포레스트.
...
- `:root`가 밝은 기본값을 깔고 각 테마 블록은 달라지는 토큰만 덮는다.
  어두운 테마들(`ink-dark` · `midnight` · `forest`)은 상태색·그림자를 **공통 블록에서 함께**
  덮는다 — 밝은 배경용 값이 검은 배경에서 무너지기 때문이다.
