...
- **조직** — 만든 사람이 방장. 이메일로 팀원 초대, 역할(관리자) 지정, 내보내기.
  받은 초대는 상단 조직 이름을 눌러 그 자리에서 수락/거절
- **보드** — 멤버마다 컬럼 하나. 모바일은 한 화면에 한 명씩, 좌우로 밀어 팀원을 오간다.
  내 할 일이 항상 처음에 보이고, 끝까지 가면 처음으로 이어진다(순환)
- **대시보드** — 모든 팀원의 할 일을 세로로 쌓아 위아래 스크롤로 한 번에 훑는 모드
- **달력** — 조직 전체 할 일을 마감일 기준으로. 날짜를 누르면 그 날 마감인 일이 담당자와 함께 나온다
- **조직 일정** — 달력에서 기간 일정을 추가한다. 고른 색이 시작일부터 종료일까지 띠로 이어지고,
  날짜별 목록에도 뜬다. 할 일과는 다른 것이라 보드에는 나오지 않는다
...
- **테마** — 잉크 화이트(기본) / 잉크 블랙
- **아바타·조직 아이콘** — 이미지 업로드(256px webp로 줄여서 저장) 또는 첫 글자 + 12색 중 하나
