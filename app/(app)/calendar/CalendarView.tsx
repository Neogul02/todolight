 ...
 * 위쪽을 0.5에서 끊는 이유: 그보다 더 올리면 달력 칸이 날짜 숫자도 겨우 들어가는 높이로
 * 눌려서 달력이라기보다 찌그러진 격자로 보인다. 목록을 그만큼 크게 보고 싶으면 그건
 * 달력 화면이 할 일이 아니다.
 */
const PANEL_SNAPS = [0.24, 0.5];
...
export function CalendarView({
  ...
  */
   ...
   * 날짜를 탭하면 **그 날짜만 고른다.**
   *
   * 예전엔 목록이 살짝만 보이는 상태에서 날짜를 누르면 목록을 반쯤 펴 줬다. 그런데
   * 달력에서 날짜를 누르는 건 대부분 "이 날 뭐 있나" 훑는 동작이라, 누를 때마다 달력이
   * 줄고 목록이 올라오면 보려던 달력이 사라진다 — 높이는 손잡이를 잡은 사람만 바꾼다.
   */
